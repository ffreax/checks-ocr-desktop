...
public class DeSkew {
    public static double skewRadians(BufferedImage image) {
    ...
    private static double findSkew(final BufferedImage img) {
        ...
        for (int row = 0; row < img.getHeight(); row++) {
            for (int col = 0; col < byteWidth; col++) {
                ...
                elem = BitUtils.INV_BITS[elem]; // Change the bit order
    ...
    private static void radon(final int width, final int height, final DataBuffer buffer, final int sign,
                      final int sharpness[]) {
        ...
        for (row = 0; row < h; row++) {
            ...
            for (column = 0; column < w; column++) {
                if (sign > 0) {
                    ...
                    p1_[h * column + row] = BitUtils.BIT_COUNT[b];
                } else {
    ...
    private static class BitUtils {
        ...
        private static final int[] INV_BITS = new int[256];
